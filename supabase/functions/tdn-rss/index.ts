/**
 * Proxies Thoroughbred Daily News main WordPress RSS so the browser can read it without CORS issues.
 * Fixed URL only — not a generic open proxy.
 */
import { getCorsHeaders } from "../_shared/cors.ts";

const TDN_FEED = "https://www.thoroughbreddailynews.com/feed/";
...
Deno.serve(async (req) => {
  ...
  try {
    const upstream = await fetch(TDN_FEED, {
      headers: {
